...
The OPGSP (Online Payment Gateway Service Providers) Integration is the process of connecting your website or online store to the EximPe payment gateway for cross-border payments. It is important because it enables merchants to accept and process payments from customers in different countries and currencies.
...
- If you’re new, register on [EximPe’s website](#) and complete the onboarding process.  
...
### What is the UDF Update API?
UDFs (User-Defined Fields) are used to capture buyer information (e.g., name, address) required for compliance and settlement in cross-border flows.

### What is the format of UDF data?
- In **EximPe Biz**: JSON body (example to be added)  
- In **EximPe Hub**: JSON body (example to be added)

### What is the maximum size of UDF data?
Maximum size is **4 KB**.

### Can I update UDF data after upload?
Yes, UDF data can be updated multiple times. The latest update will overwrite previous data.
