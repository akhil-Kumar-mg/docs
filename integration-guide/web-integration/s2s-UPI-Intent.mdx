...
<CardGroup cols={3} className="gap-4 items-stretch">
  <Card
    className="h-full"
    title="No Redirect Hops"
    icon="arrow-path"
    href="#step-1-initiate-payment"
  >
    Initiate transactions directly from your server to improve user experience
  </Card>
  <Card
    className="h-full"
    title="Flexible Intent Invocation"
    icon="smartphone"
    href="#step-2-invoke-upi-intent"
  >
    Support both generic and app-specific UPI intents on mobile web and Android apps
  </Card>
  <Card
    className="h-full"
    title="Robust Server Callbacks"
    icon="refresh-cw"
    href="#step-4-server-to-server-callback"
  >
