...
You can use anything@payu or 9999999999@payu as VPA to test your UPI-related integration in the sandbox (Test environment) only for the payment flows involving _payment AP.

<Note>
Only `anything@payu` is supported for UPI transactions in the **sandbox** or **API Playground** when using the `_orders` API. Other VPAs will not work in the test environment.
</Note>
